<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */

    public function up()
    {
        Schema::create('articles', function (Blueprint $table) {
            $table->id();
            $table->string('title');
            $table->string('slug')->unique()->default('default-slug');;
            $table->text('content');
            $table->text('excerpt')->nullable();  // Ringkasan artikel  // Relasi dengan tabel categories
            $table->timestamp('published_at')->nullable();  // Tanggal publikasi
            $table->enum('category', ['news', 'stories', 'campaign', 'education', 'event', 'achievements', 'social'])->default('news');
            $table->enum('status', ['draft', 'published', 'archived'])->default('draft');
            $table->enum('featured', ['primary', 'hot', 'none'])->default('none');
            $table->string('image')->nullable();  // Path gambar artikel
            $table->timestamps();
        });
    }


    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('articles');
    }
};
